...
LoRA는 dense layer가 존재하는 모든 신경망에 적용이 가능하지만, 여기서는 language modeling에 초점을 맞추고 있다.<br>
먼저 \\(\Phi\\) 로 parameterize 되어있는 pretrained autoregressive language model \\(P\Phi(y|x)\\) 이 주어졌다고 가정한다.
\\(P\Phi(y|x)\\) 는 GPT와 같은 일반적인 multi-task learner일 수 있다.<br>
...
full fine-tuning의 경우 위처럼 모델은 pretrained weights \Phi_0 으로 초기화되고 conditional language modeling objective를 최대화하기 위해 기울기를 반복적으로 \\(\Phi_0 + \triangle\Phi\\)로 업데이트한다.<br>
...
따라서 \\(|\triangle\Phi|\\)를 찾는 작업은 \Theta에 대한 optimization으로 대체된다.<br>
